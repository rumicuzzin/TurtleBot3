...
#include "rosidl_runtime_c/string.h"  // robot_mode
#include "rosidl_runtime_c/string_functions.h"  // robot_mode
  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  {
    const rosidl_runtime_c__String * str = &ros_message->robot_mode;
  ...
  shared_interfaces__srv__NavigationMode_Request * ros_message)
{
  // Field name: robot_mode
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->robot_mode.data) {
      rosidl_runtime_c__String__init(&ros_message->robot_mode);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->robot_mode,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'robot_mode'\n");
      return false;
    }
  }
